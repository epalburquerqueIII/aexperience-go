...
func UsuarioList(w http.ResponseWriter, r *http.Request) {
	...
	db := database.DbConn()
	selDB, err := db.Query("SELECT usuarios.id, usuarios.nombre, nif, email, usuarios_roles.nombre, tipo, telefono, sesionesbonos, newletter, fechaBaja  FROM usuarios left outer join usuarios_roles on (tipo = usuarios_roles.id) " + jtsort)
	if err != nil {
