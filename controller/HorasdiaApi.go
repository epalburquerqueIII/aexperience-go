...
func HorasReservables(fechabusqueda string, espacio string) []model.THorasdia {
	...
	db := database.DbConn()
	selDB, err := db.Query("SELECT horarios.id, horarios.idespacio,horarios.hora, IF(isnull(reservas.hora),0,1) as reservado from horarios left outer join reservas on reservas.fecha = '" + fechabusqueda + "' and reservas.hora = horarios.hora and horarios.idespacio = reservas.idespacio where horarios.fechaInicio >= '2019-10-01' and horarios.fechaFin <= '2019-12-31' and horarios.idespacio = '" + espacio + "' order by hora")
