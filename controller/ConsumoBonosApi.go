...
//RestarSesionBono quita las sesiones pertinentes del bono del usuario al ser usado
func RestarSesionBono(w http.ResponseWriter, r *http.Request) {
	db := database.DbConn()
	usu := model.Tusuario{}
	consu := model.Tconsumo{}
	if r.Method == "POST" {
		...
		selDB, err := db.Query("SELECT sesionesBonos from usuarios Order by usuarios.nombre")
		if err != nil {
			panic(err.Error())
		}
		for usu.SesionesBonos > 0 {
			selDB, err := db.Query("SELECT sesiones from consumoBonos Order by usuarios.nombre")
			if err != nil {
				panic(err.Error())
			}
			usu.SesionesBonos - consu.Sesiones
			insForm, err := db.Prepare("UPDATE usuarios SET sesiones=? WHERE usuarios.id=?")
		}
